...
public abstract class AbstractFileProcessor
{
    ...
    {
        if (!file.isWritable() && !FileDocumentManager.fileForDocumentCheckedOutSuccessfully(
            PsiDocumentManager.getInstance(myProject).getDocument(file), myProject))
        {
            Messages.showMessageDialog(myProject, "Cannot modify process read-only file.", "File Is Read-Only",
    ...
    private void handleFiles(String msg, final List<PsiFile> files)
    {
        ...
        do
        {
            ...
            int res = Messages.showOkCancelDialog(myProject,
                msg + " contains read-only file(s).\nProcess all other files?",
                "Cannot Modify Read-Only Files", Messages.getQuestionIcon());
            if (res != 0)
            {
                files.clear();
                break;
